...
export default class NewReq extends Component {
    ...
    hospitalChangeHandler = event => {
        this.setState({
            ...this.state,
            enteredHospital : event.target.value
        });
    }

    checkHospitalIsValid = () => {
        const {enteredHospital} = this.state;
        let x = false;
    ...
    render(){
        return(
            ...
            <form className={styles.form} method='POST' action='/'>
                ...
                }} onChange={this.hospitalChangeHandler} value={this.state.enteredHospital} onBlur={this.checkHospitalIsValid} />

                <div style={{paddingLeft: 1.2+"vw"}}>
                    <RMapGL
                        center={{ lat: 0, long: 0 }}
                        dimentions={{ height: 43, width: 41 }}
                        geocoder={{ top: 122+"vh" , right: 30+"vw" }}
